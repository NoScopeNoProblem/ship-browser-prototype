function enemyIntentDistribution(){
  const hits = new Map(), dodges = new Map(), targetMap = new Map(), missMap = [];
  ...
  return {hits, dodges, targetMap, missMap};
}
...
function renderPipsAndIntentChips(){
  ...
  const enemyState = enemyIntentDistribution();
  const playerState = playerIntentDistribution();
  playerRooms.forEach(r=>{
    const el = getEntityElement('player',r.id);
    el.querySelector('.pips').innerHTML = pipsMarkup(r, enemyState.hits.get(r.id)||0, enemyState.dodges.get(r.id)||0);
    if(enemyState.hits.get(r.id)) el.classList.add('enemy-intended');
    if(enemyState.dodges.get(r.id)) el.classList.add('dodged');
    if(enemyState.targetMap.has(r.id)) addEnemyIntentStack(el, enemyState.targetMap.get(r.id));
  });
  playerMastPips.innerHTML = pipsMarkup(playerMast, enemyState.hits.get(playerMast.id)||0, enemyState.dodges.get(playerMast.id)||0);
  ...
  if(enemyState.targetMap.has(playerMast.id)) addEnemyIntentStack(playerMastBox, enemyState.targetMap.get(playerMast.id));
  enemyState.missMap.forEach(addMissMarker);

  enemyRooms.forEach(r=>{
    const el = getEntityElement('enemy',r.id);
    el.querySelector('.pips').innerHTML = pipsMarkup(r, playerState.hits.get(r.id)||0, 0);
    if(playerState.targetMap.has(r.id)) addFriendlyIntentStack(el, playerState.targetMap.get(r.id));
  });
  enemyMastPips.innerHTML = pipsMarkup(enemyMast, playerState.hits.get(enemyMast.id)||0, 0);
  if(playerState.targetMap.has(enemyMast.id)) addFriendlyIntentStack(enemyMastBox, playerState.targetMap.get(enemyMast.id));
}
function addEnemyIntentStack(targetEl, intents){
  const stack = document.createElement('div'); stack.className='intent-stack';
  intents.forEach(intent=>{
    const src = sourceEntity('enemy', intent.sourceId);
    const chip = document.createElement('div'); chip.className='intent-chip';
    chip.innerHTML = iconMarkup(src.weapon, enemySuffix[src.id]||'', true);
    chip.addEventListener('mouseenter',(e)=>{e.stopPropagation(); state.hoverIntent = {side:'enemy', sourceId:intent.sourceId}; refresh();});
    chip.addEventListener('mouseleave',()=>{ state.hoverIntent = null; refresh();});
...
function addFriendlyIntentStack(targetEl, intents){
  ...
  intents.forEach(intent=>{
    ...
    const chip = document.createElement('div'); chip.className='intent-chip friendly';
    chip.innerHTML = iconMarkup(src.weapon, playerSuffix[src.id]||'', true);
    chip.addEventListener('mouseenter',(e)=>{e.stopPropagation(); state.hoverIntent = {side:'player', sourceId:intent.sourceId}; refresh();});
    chip.addEventListener('mouseleave',()=>{ state.hoverIntent = null; refresh();});
    stack.appendChild(chip);
  });
  targetEl.appendChild(stack);
...
function addMissMarker(intent){
  ...
  marker.addEventListener('mouseenter',()=>{ state.hoverIntent = {side:'enemy', sourceId:intent.sourceId}; refresh(); });
  marker.addEventListener('mouseleave',()=>{ state.hoverIntent = null; refresh(); });
...
function highlightOverview(side){
  ...
  if(side==='enemy'){
    enemyIntents.forEach(intent=>{
      ...
      if(impact) getEntityElement('player', impact.id).classList.add('enemy-intended');
    });
    weaponInfo.textContent = 'Enemy firing arcs';
    targetInfo.textContent = 'Red rooms show where enemy intent currently lands.';
  } else {
    ...
    targetInfo.textContent = 'Blue rooms are currently in range of at least one of your gun decks.';
  }
}
...
function renderSelection(){
  ...
  targetInfo.textContent = 'Click a highlighted enemy room or mast to assign this shot. Click empty space to cancel.';
}
function renderHoverWeapon(){
  ...
  if(state.hoveredWeapon.side==='enemy'){
    ...
    targetInfo.textContent = impact ? `${impact.name}: ${intent.damage} damage if you do not move or block.` : 'This shot currently misses.';
  } else {
...
function renderHoverIntent(){
  ...
  const sourceEl = getEntityElement(state.hoverIntent.side, src.id); if(sourceEl) sourceEl.classList.add('focus-source');
  if(state.hoverIntent.side==='enemy'){
    ...
    targetInfo.textContent = impact ? `${impact.name}: ${intent.damage} damage` : 'This shot currently misses due to movement.';
  } else {
...
function refresh(){
  ...
  if(!state.hoveredWeapon && !state.hoverIntent && !state.selectedWeaponId && !state.overview){
    ...
    targetInfo.textContent = 'Hover a cannon, or click one of your gun decks to plan a shot.';
  }
}
