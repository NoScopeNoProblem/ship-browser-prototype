(() => {
  ...
  tracerOverlay.setAttribute('class','v11-intent-tracer-overlay');
  ...
  playerTraceOverlay.setAttribute('class','v11-player-tracer-overlay');
  playerTraceOverlay.setAttribute('aria-hidden','true');
  stage.appendChild(playerTraceOverlay);

  const movementOverlay=document.createElementNS('http://www.w3.org/2000/svg','svg');
  movementOverlay.setAttribute('class','v11-movement-overlay');
  movementOverlay.setAttribute('aria-hidden','true');
  stage.appendChild(movementOverlay);
  ...
  const turnEl=previewOverlay.querySelector('.v11-intent-turn span');
  const titleEl=previewOverlay.querySelector('.v11-intent-title');
  const detailEl=previewOverlay.querySelector('.v11-intent-detail');
  ...
  function adjacent(source,target){return source&&target&&source.id!==target.id&&Math.abs(source.col-target.col)<=1&&Math.abs(source.row-target.row)<=1;}

  function buildEnemyActions(){
    provisionalOriginals.clear();
    deferredLoadProgress.clear();
    const turn=currentTurn();
    const actions=[];

    enemyRooms.filter(r=>r.actionType==='repair'&&r.hp>0).sort((a,b)=>(a.col-b.col)||(a.row-b.row)).forEach(source=>{
      const targets=enemyRooms.filter(r=>r.hp>0&&r.hp<r.max).sort((a,b)=>{
      ...
      if(targets[0]) actions.push({id:`repair-${turn}-${++actionSerial}`,turn,sourceId:source.id,targetId:targets[0].id,actionType:'repair'});
    });

    const assigned=new Set();
    enemyRooms.filter(r=>r.actionType==='quickLoad'&&r.hp>0).sort((a,b)=>(a.col-b.col)||(a.row-b.row)).forEach(source=>{
      const targets=enemyRooms.filter(r=>r.weapon&&r.hp>0&&!assigned.has(r.id)&&adjacent(source,r)&&stateFor(r)?.mode==='loading').sort((a,b)=>{
        const damage=(weapons[b.weapon]?.damage||0)-(weapons[a.weapon]?.damage||0); if(damage)return damage;
        return (a.col-b.col)||(a.row-b.row);
      });
      if(targets[0]){
        const target=targets[0]; assigned.add(target.id);
        const original=stateFor(target); provisionalOriginals.set(target.id,{...original});
        actions.push({id:`quick-${turn}-${++actionSerial}`,turn,sourceId:source.id,targetId:target.id,actionType:'quickLoad'});
      }
    });

    window.enemyActionIntents=actions;
    syncProvisionalQuickLoads();
    refresh();
  }
  ...
  function syncProvisionalQuickLoads(){
    ...
    currentActions().filter(a=>a.actionType==='quickLoad').forEach(action=>{
      const target=enemyRoom(action.targetId),original=provisionalOriginals.get(action.targetId);
      if(!target||!original)return;
      const cancelled=utilityIntentCancelled(action);
      if(cancelled){combatTurn.setWeaponState(target.id,{...original});action.provisionalActive=false;}
  ...
  enemyIntentDistribution=function(playerState=playerIntentDistribution()){
    const actions=currentActions().filter(a=>a.actionType==='quickLoad');
    const temporarilyReady=[];
  ...
  function applyLayout(){
    const enemyTop=104,trackTop=enemyTop+(ENEMY_SHIP_SETUP.rows*ROOM_H())+58,playerTop=trackTop+156,minHeight=playerTop+(PLAYER_SHIP_SETUP.rows*ROOM_H())+175;
    enemyBlock.style.setProperty('top',`${enemyTop}px`,'important');
    trackRow.style.setProperty('top',`${trackTop}px`,'important');
  ...
  function renderQuickLoadMarkers(){
    ...
    playerQuickLoaded.forEach((turn,id)=>{if(turn!==currentTurn())return;const room=playerRooms.find(r=>r.id===id),el=room&&getEntityElement('player',id);if(el){const b=document.createElement('div');b.className='v11-quickloaded-badge player';b.textContent='↻+';b.title='Quick Loaded';el.appendChild(b);}});
    enemyQuickLoaded.forEach((turn,id)=>{if(turn!==currentTurn())return;const room=enemyRoom(id),el=room&&getEntityElement('enemy',id);if(el){const b=document.createElement('div');b.className='v11-quickloaded-badge enemy';b.textContent='↻+';b.title='Quick Loaded';el.appendChild(b);}});
  }
  ...
  refresh=function(){
    ...
    const revealed=revealAnyDamagedUnknowns();if(revealed.length)renderShips();previousRefresh();applyLayout();decorateThreatDensity();decoratePreviewConcealment();decorateOverviewSources();renderEnemyUtilityIntents();renderQuickLoadMarkers();renderMovementArrow();
  ...
  async function animateEnemyTrace(source,intent,impact,token){
    tracerOverlay.innerHTML='';const sourceEl=getEntityElement('enemy',source.id);if(!sourceEl)return;const a=center(sourceEl),b=endpoint(intent,impact),line=document.createElementNS('http://www.w3.org/2000/svg','line');line.setAttribute('x1',a.x);line.setAttribute('y1',a.y);line.setAttribute('x2',b.x);line.setAttribute('y2',b.y);line.setAttribute('pathLength','1');line.setAttribute('class',`v11-intent-tracer${impact?'':' miss'}`);tracerOverlay.appendChild(line);sourceEl.classList.add('v11-preview-source');await wait(390);if(token!==preview.token)return;sourceEl.classList.remove('v11-preview-source');
  }
  ...
  function schedulePreview(delay=980){
    clearTimeout(preview.scheduled);++preview.token;preview.running=false;preview.active=true;preview.revealedSources.clear();preview.revealedUtilities.clear();document.body.classList.add('v11-intent-preview-active');refresh();preview.scheduled=setTimeout(()=>{preview.active=false;runPreview();},delay);
  }
  ...
  stage.addEventListener('click',e=>{
    ...
    const roomEl=e.target.closest&&e.target.closest('.room[data-side="enemy"]');const mast=e.target.closest&&e.target.closest('#enemyMastBox');const targetId=roomEl?.dataset.id||(mast?enemyMast.id:null);if(!targetId)return;
    setTimeout(()=>{
      if(state.playerIntents?.[selected]!==targetId)return;animatePlayerTarget(selected,targetId);const target=sourceEntity('enemy',targetId);if(target?.weapon&&target.hp-plannedDamageTo(target.id)<=0&&activeEnemyIntent(target.id)&&currentlyReady(target))showRoomTooltip(getEntityElement('enemy',target.id),'Cancelled');
  ...
  stage.addEventListener('click',e=>{
    ...
    setTimeout(()=>{
      if(source.actionType==='repair'&&target.hp>beforeHp)repairPulse(getEntityElement('player',target.id));
      if(source.actionType==='quickLoad'&&beforeState?.mode==='loading'&&currentlyReady(target)){playerQuickLoaded.set(target.id,currentTurn());refresh();}
    },0);
  },true);
  ...
  async function resolveBeforeEnemyFire({logLine,floatNote,wait:phaseWait}){
    for(const action of currentActions().sort((a,b)=>{const sa=enemyRoom(a.sourceId),sb=enemyRoom(b.sourceId);return((sa?.col??999)-(sb?.col??999))||((sa?.row??999)-(sb?.row??999));})){
      const source=enemyRoom(action.sourceId),target=enemyRoom(action.targetId),targetEl=target&&getEntityElement('enemy',target.id);const sourceDead=!source||source.hp<=0,targetDead=!target||target.hp<=0;
      if(sourceDead||targetDead){
        if(action.actionType==='quickLoad'&&target){const current=stateFor(target),original=provisionalOriginals.get(target.id);if(original&&current?.mode==='ready'){combatTurn.setWeaponState(target.id,{...original});deferredLoadProgress.add(target.id);}}
        if(targetEl){showRoomTooltip(targetEl,'Cancelled');await floatNote(targetEl,'CANCELLED','disabled');}
        logLine(`<span class="v3-log-enemy">Enemy ${action.actionType==='repair'?'repair':'Quick Load'}</span> cancelled.`);continue;
      }
      if(action.actionType==='repair'){
        if(target.hp>=target.max){showRoomTooltip(targetEl,'Cancelled');continue;}target.hp=Math.min(target.max,target.hp+(source.repairAmount||1));renderShips();refresh();repairPulse(getEntityElement('enemy',target.id));logLine(`<span class="v3-log-enemy">Enemy repair</span> restores 1 blip to ${target.name}.`);await phaseWait(360);
      }else{
  ...
  async function afterEnemyFire(){
    deferredLoadProgress.forEach(id=>{const s=stateFor(roomById(id));if(!s||s.mode!=='loading')return;const remaining=(s.remaining||1)-1;if(remaining<=0)combatTurn.setReady(id);else combatTurn.setWeaponState(id,{...s,remaining});});deferredLoadProgress.clear();
  }

  window.enemyAI={
    rebuildForTurn:buildEnemyActions,
    currentActions,
    utilityIntentCancelled,
    resolveBeforeEnemyFire,
    afterEnemyFire,
    get previewActive(){return preview.active||preview.running;},
