(() => {
  const utility = { selected:null, used:{}, rollback:[], wasResolving:document.body.classList.contains('v3-resolving') };
  ...
  // Carpenter may repair itself or any of the eight surrounding rooms.
  function carpenterReach(source,target){ const {dx,dy}=delta(source,target); return Math.abs(dx)<=1&&Math.abs(dy)<=1; }
  // Magazine Quick Load remains strictly orthogonal and never targets itself.
  ...
  function applyUtility(sourceId,targetId){
    ...
    if(source.actionType==='repair'){
      utility.rollback.push({type:'repair',targetId,hp:target.hp});target.hp=Math.min(target.max,target.hp+(source.repairAmount||1));
    }else if(source.actionType==='quickLoad'){
      if(!window.combatTurn)return false;utility.rollback.push({type:'reload',targetId,weaponState:combatTurn.getWeaponState(targetId)});combatTurn.setReady(targetId);target.loading=false;
    }
    utility.used[sourceId]=true;utility.selected=null;state.hoveredWeapon=null;state.hoverIntent=null;refresh();
  ...
  function rollbackUtilityEffects(){
    ...
    utility.rollback=[];utility.used={};utility.selected=null;
  }
  ...
  function decorateUtilities(){
    ...
    playerRooms.forEach(target=>{
      ...
      // On the Carpenter itself, the green repair-heart is clearer than a directional arrow.
  ...
  stage.addEventListener('click',e=>{
    ...
    if(utility.selected){
      ...
      if(target&&source&&target.id===source.id){
        ...
        utility.selected=null;refresh();
        if(source.actionType==='repair'&&target.hp>=target.max)tooltip(roomEl,'No repair needed');
        return;
      }
  ...
  function retargetEnemyIntentsInRange(){
    ...
    // Mast is a normal legal target. To keep the AI readable rather than random, one
    // ready gun deliberately tests the Mast on alternating turns whenever its range permits.
    const turn=window.combatTurn?.turn||1;
    let mastGunId=null;
  ...
  const observer=new MutationObserver(()=>{const now=document.body.classList.contains('v3-resolving');if(!utility.wasResolving&&now)utility.selected=null;if(utility.wasResolving&&!now){state.turnStartMast=state.playerMastTrack;utility.used={};utility.rollback=[];utility.selected=null;retargetEnemyIntentsInRange();refresh();}utility.wasResolving=now;});observer.observe(document.body,{attributes:true,attributeFilter:['class']});
