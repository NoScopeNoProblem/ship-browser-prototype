(() => {
  const utility = { selected:null, used:{}, rollback:[], quickLoaded:{}, braced:{}, wasResolving:document.body.classList.contains('v3-resolving') };
  ...
  function entityById(id){ return id===playerMast.id?playerMast:roomById(id); }
  function isUtilityRoom(room){ return !!room && utilityTypes.has(room.actionType); }
  ...
  function carpenterReach(source,target){ if(target?.kind==='mast')return false;const {dx,dy}=delta(source,target);return Math.abs(dx)<=1&&Math.abs(dy)<=1; }
  function magazineReach(source,target){ if(target?.kind==='mast')return false;const {dx,dy}=delta(source,target);return target.id!==source.id&&Math.abs(dx)+Math.abs(dy)===1; }
  function boatswainReach(source,target){ if(target?.kind==='mast')return true;const {dx,dy}=delta(source,target);return Math.abs(dx)<=1&&Math.abs(dy)<=1; }
  function inReach(source,target){
    if(!source||!target)return false;
    if(source.actionType==='repair')return carpenterReach(source,target);
    if(source.actionType==='quickLoad')return magazineReach(source,target);
    if(source.actionType==='resetSails')return target.id===playerMast.id;
    if(source.actionType==='brace')return boatswainReach(source,target);
    return false;
  }
  function reachableRooms(sourceOrId){
    const source=typeof sourceOrId==='string'?roomById(sourceOrId):sourceOrId;if(!source)return[];
    return [...playerRooms,playerMast].filter(r=>inReach(source,r));
  }
  function isBraced(id){const b=utility.braced[id];return !!b&&b.turn===currentTurn()&&!b.consumed;}
  function eligibleTargets(sourceOrId){
    const source=typeof sourceOrId==='string'?roomById(sourceOrId):sourceOrId;if(!source)return[];
    if(source.actionType==='repair')return reachableRooms(source).filter(r=>r.kind!=='mast'&&r.hp>0&&r.hp<r.max);
    if(source.actionType==='quickLoad')return reachableRooms(source).filter(r=>r.weapon&&r.hp>0&&weaponLoading(r));
    if(source.actionType==='resetSails')return playerMast.hp>0&&window.combatManoeuvre?.cooldown?[playerMast]:[];
    if(source.actionType==='brace')return reachableRooms(source).filter(r=>r.hp>0&&!isBraced(r.id));
    return[];
  }
  function arrowFor(source,target){
    if(target.kind==='mast')return source.actionType==='resetSails'?'⛵':'⛨';
    if(source.id===target.id)return source.actionType==='brace'?'⛨':'♥';
  ...
  function noTargetText(actionType){
    return actionType==='repair'?'Nothing to repair':actionType==='quickLoad'?'No adjacent guns loading':actionType==='resetSails'?'Sails are already ready':'No unbraced target in reach';
  }
  function sourceLabel(room){
    return room.actionType==='repair'?'Carpenter':room.actionType==='quickLoad'?'Magazine — Quick Load':room.actionType==='resetSails'?'Sailmaster — Reset Sails':'Boatswain — Brace';
  }
  function instruction(room){
    return room.actionType==='repair'?'Self or adjacent rooms (including diagonals). Select a damaged room.':
      room.actionType==='quickLoad'?'Orthogonally adjacent rooms only. Select a loading gun.':
      room.actionType==='resetSails'?'Select the Mast to finish Resetting Sails immediately.':
      'Select self, an adjacent room, or the Mast. Brace prevents the first 1 incoming damage this turn.';
  }
  ...
  function selectUtility(id,el){
    ...
    if(!targets.length){clearUtilitySelection();tooltip(el,noTargetText(room.actionType));refresh();return;}
  ...
  function applyUtility(sourceId,targetId){
    const source=roomById(sourceId),target=entityById(targetId);if(!source||!target||wasUsed(sourceId))return false;
    ...
    }else if(source.actionType==='resetSails'){
      if(!window.combatManoeuvre?.cooldown)return false;
      utility.rollback.push({type:'resetSails',turn,sourceId,targetId,cooldown:true});
      if(!combatManoeuvre.resetSails?.())return false;
    }else if(source.actionType==='brace'){
      utility.rollback.push({type:'brace',turn,sourceId,targetId});
      utility.braced[targetId]={turn,sourceId,consumed:false};
    }
    utility.used[sourceId]=turn;utility.selected=null;state.hoveredWeapon=null;state.hoverIntent=null;refresh();
    const message=source.actionType==='repair'?'Repaired +1':source.actionType==='quickLoad'?'Quick Load':source.actionType==='resetSails'?'Sails reset — movement ready':'Braced — first 1 damage prevented';
    tooltip(getEntityElement('player',targetId),message);return true;
  }

  function rollbackUtilityEffects(){
    let restoreCooldown=null;
    for(let i=utility.rollback.length-1;i>=0;i--){
      const a=utility.rollback[i];
      if(a.type==='repair'){const t=roomById(a.targetId);if(t)t.hp=a.hp;}
      else if(a.type==='reload'&&window.combatTurn)combatTurn.setWeaponState(a.targetId,a.weaponState);
      else if(a.type==='resetSails')restoreCooldown=!!a.cooldown;
    }
    utility.rollback=[];utility.used={};utility.quickLoaded={};utility.braced={};utility.selected=null;
    if(restoreCooldown!==null&&window.combatManoeuvre?.setCooldown)combatManoeuvre.setCooldown(restoreCooldown);
  }
  ...
  function ensurePlanningQuickLoads(){
    ...
    Object.entries(utility.quickLoaded).forEach(([targetId,stamp])=>{
      ...
      if(!combatTurn.isReady(targetId))combatTurn.setReady(targetId);target.loading=false;
    });
  }
  ...
  stage.addEventListener('click',e=>{
    ...
    const mastEl=e.target.closest&&e.target.closest('#playerMastBox');
    const id=roomEl?.dataset.id||(mastEl?playerMast.id:null);
    if(utility.selected){
      e.preventDefault();e.stopImmediatePropagation();const source=roomById(utility.selected),target=id?entityById(id):null,targetEl=roomEl||mastEl;
      if(target&&source&&target.id===source.id){if((source.actionType==='repair'&&target.hp>0&&target.hp<target.max)||source.actionType==='brace'){const selected=utility.selected;if(applyUtility(selected,id))return;}utility.selected=null;refresh();if(source.actionType==='repair'&&target.hp>=target.max)tooltip(targetEl,'No repair needed');return;}
      if(target&&!inReach(source,target)){utility.selected=null;refresh();tooltip(targetEl,'Out of reach');return;}
      if(source?.actionType==='repair'&&target&&target.hp<=0){utility.selected=null;refresh();tooltip(targetEl,'Beyond repair');return;}
      if(source?.actionType==='repair'&&target&&target.hp>=target.max){utility.selected=null;refresh();tooltip(targetEl,'No repair needed');return;}
      if(source?.actionType==='quickLoad'&&target&&(!target.weapon||!weaponLoading(target))){utility.selected=null;refresh();tooltip(targetEl,target?.weapon?'Already loaded':'Cannot Quick Load');return;}
      if(source?.actionType==='resetSails'&&target?.id!==playerMast.id){utility.selected=null;refresh();tooltip(targetEl,'Reset Sails targets the Mast');return;}
      if(source?.actionType==='brace'&&target&&isBraced(target.id)){utility.selected=null;refresh();tooltip(targetEl,'Already braced');return;}
  ...
  const observer=new MutationObserver(()=>{const now=document.body.classList.contains('v3-resolving');if(!utility.wasResolving&&now)utility.selected=null;if(utility.wasResolving&&!now){state.turnStartMast=state.playerMastTrack;utility.used={};utility.rollback=[];utility.quickLoaded={};utility.braced={};utility.selected=null;retargetEnemyIntentsInRange();refresh();}utility.wasResolving=now;});observer.observe(document.body,{attributes:true,attributeFilter:['class']});
  ...
  function currentSupportPlans(){const turn=currentTurn();return utility.rollback.filter(a=>a.turn===turn).map(a=>({sourceId:a.sourceId,targetId:a.targetId,actionType:a.type==='repair'?'repair':a.type==='reload'?'quickLoad':a.type}));}
  function playerBraceCount(id){return isBraced(id)?1:0;}
  function consumePlayerBrace(id,amount=1){const b=utility.braced[id];if(!b||b.turn!==currentTurn()||b.consumed||amount<=0)return 0;b.consumed=true;return Math.min(1,amount);}

  window.combatUtility={
    eligibleTargets,reachableRooms,inReach,isUsed:id=>wasUsed(id),
    isAvailable(room){return isUtilityRoom(room)&&room.hp>0&&!wasUsed(room.id)&&eligibleTargets(room).length>0;},
    plannedRepairs(){const turn=currentTurn();return utility.rollback.filter(a=>a.type==='repair'&&a.turn===turn).map(a=>({sourceId:a.sourceId,targetId:a.targetId,beforeHp:a.hp,afterHp:roomById(a.targetId)?.hp??a.hp}));},
    quickLoadedTargets(){const turn=currentTurn();return Object.entries(utility.quickLoaded).filter(([,stamp])=>stamp===turn).map(([id])=>id);},
    bracePlans(){const turn=currentTurn();return Object.entries(utility.braced).filter(([,b])=>b.turn===turn&&!b.consumed).map(([targetId,b])=>({sourceId:b.sourceId,targetId,actionType:'brace'}));},
    supportPlans:currentSupportPlans,
    get selected(){return utility.selected;}
  };
  window.combatBrace={
    count(side,id){return side==='player'?playerBraceCount(id):Number(window.enemyAI?.braceCount?.(id)||0);},
    consume(side,id,amount=1){return side==='player'?consumePlayerBrace(id,amount):Number(window.enemyAI?.consumeBrace?.(id,amount)||0);}
  };
