(() => {
  ...
  // Utility use is turn-stamped rather than a sticky boolean. This prevents stale USED state
  // surviving a turn transition or being confused with movement/threat state.
  ...
  function applyUtility(sourceId,targetId){
    ...
    if(source.actionType==='repair'){
      ...
      utility.rollback.push({type:'repair',turn,sourceId,targetId,hp:before});
      ...
      utility.rollback.push({type:'reload',turn,sourceId,targetId,weaponState:combatTurn.getWeaponState(targetId)});
      combatTurn.setReady(targetId);target.loading=false;utility.quickLoaded[targetId]=turn;
    }
  ...
  // A Quick Load is a planning-phase state change. Moving the ship must not silently undo it.
  // Reassert it while planning; once resolution starts, the normal weapon cadence owns state.
  function ensurePlanningQuickLoads(){
    if(resolving()||!window.combatTurn)return;
    const turn=currentTurn();
    Object.entries(utility.quickLoaded).forEach(([targetId,stamp])=>{
      if(stamp!==turn)return;
      const target=roomById(targetId);if(!target||target.hp<=0)return;
      const rollback=utility.rollback.find(a=>a.type==='reload'&&a.turn===turn&&a.targetId===targetId);
      if(!rollback||!wasUsed(rollback.sourceId))return;
  ...
  function decorateUtilities(){
    ensurePlanningQuickLoads();
    document.querySelectorAll('.utility-used-badge').forEach(n=>n.remove());clearReachDecorations();
    playerRooms.filter(isUtilityRoom).forEach(room=>{
      const el=getEntityElement('player',room.id);if(!el)return;
      if(wasUsed(room.id)){el.classList.add('utility-used');const b=document.createElement('div');b.className='utility-used-badge';b.innerHTML='<span class="tick">✓</span> USED';el.appendChild(b);}
      else if(room.hp<=0||!eligibleTargets(room).length)el.classList.add('utility-unavailable');
    });
    ...
    playerRooms.forEach(target=>{
      ...
      el.classList.add('utility-reachable');if(eligible.has(target.id))el.classList.add('utility-target');else el.classList.add('utility-reach-inactive');
      if(target.id===source.id&&source.actionType!=='repair')return;
      const arrow=document.createElement('div');arrow.className=`utility-reach-arrow${eligible.has(target.id)?' active':''}`;arrow.textContent=arrowFor(source,target);arrow.title=eligible.has(target.id)?'Action can reach this room':'In range, but not applicable';el.appendChild(arrow);
    });
    weaponInfo.textContent=source.actionType==='repair'?'Carpenter':'Magazine — Quick Load';
    targetInfo.textContent=source.actionType==='repair'?'Self or adjacent rooms (including diagonals). Select a damaged room.':'Orthogonally adjacent rooms only. Select a loading gun.';
  }
  ...
  stage.addEventListener('click',e=>{
    ...
    if(utility.selected){
      ...
      if(target&&source&&target.id===source.id){
        if(source.actionType==='repair'&&target.hp>0&&target.hp<target.max){const selected=utility.selected;if(applyUtility(selected,id))return;}
        utility.selected=null;refresh();if(source.actionType==='repair'&&target.hp>=target.max)tooltip(roomEl,'No repair needed');return;
      }
  ...
  enemyIntentDistribution=function(playerState=playerIntentDistribution()){
    const saved=enemyIntents.slice(),active=saved.filter(intent=>!intent.inactive&&enemyIntentInRange(intent));enemyIntents.splice(0,enemyIntents.length,...active);
    try{return baseEnemyIntentDistribution(playerState);}finally{enemyIntents.splice(0,enemyIntents.length,...saved);}
  };
  ...
  function retargetEnemyIntentsInRange(){
    ...
    if(turn%2===0&&playerMast.hp>0){
      const mastShooters=enemyRooms.filter(r=>r.weapon&&r.hp>0&&window.combatTurn?.isReady(r.id)&&getTargetsForWeapon('enemy',r).some(t=>t.id===playerMast.id)).sort((a,b)=>(a.col-b.col)||(a.row-b.row));
      if(mastShooters.length)mastGunId=mastShooters[(Math.floor(turn/2)-1)%mastShooters.length].id;
    }
    enemyIntents.forEach(intent=>{
      const source=sourceEntity('enemy',intent.sourceId);if(!source||!source.weapon||source.hp<=0){intent.inactive=true;return;}
      const candidates=getTargetsForWeapon('enemy',source).filter(t=>t&&t.hp>0);if(!candidates.length){intent.inactive=true;return;}
      const previous=intent.logicalTargetId?sourceEntity('player',intent.logicalTargetId):null;
      let target=source.id===mastGunId?candidates.find(t=>t.id===playerMast.id)||null:null;
      if(!target)target=previous&&candidates.some(t=>t.id===previous.id)?previous:null;
      if(!target){const desiredRow=previous&&Number.isFinite(previous.row)?previous.row:(intent.lane==='mast'?null:intent.lane),desiredWorld=Number.isFinite(intent.targetWorld)?intent.targetWorld:sourceWorld('enemy',source);target=candidates.slice().sort((a,b)=>{const ar=Number.isFinite(a.row)?a.row:null,br=Number.isFinite(b.row)?b.row:null,aw=targetWorld(a),bw=targetWorld(b),ap=(desiredRow!==null&&ar!==desiredRow?4:0)+Math.abs(aw-desiredWorld),bp=(desiredRow!==null&&br!==desiredRow?4:0)+Math.abs(bw-desiredWorld);return ap-bp;})[0];}
      intent.inactive=false;intent.logicalTargetId=target.id;if(target.kind==='mast'){intent.lane='mast';intent.targetWorld=state.playerMastTrack;}else{intent.lane=target.row;intent.targetWorld=playerWorldCol(target.col);}
    });
  }
