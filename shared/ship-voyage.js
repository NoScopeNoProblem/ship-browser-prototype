(() => {
  ...
  function ensureState(state){
    ...
    state.shipHealth.mast.max=HEALTH_DEF.mast.max;
    state.shipHealth.mast.hp=Math.max(0,Math.min(HEALTH_DEF.mast.max,Number(state.shipHealth.mast.hp??HEALTH_DEF.mast.max)));
  ...
  function emitShip(state){try{window.dispatchEvent(new CustomEvent('adventure-ship-changed',{detail:{shipHealth:clone(state.shipHealth),scoreStats:clone(state.scoreStats)}}));}catch{}}
  function update(mutator){let result;const state=Adventure.update(s=>{ensureState(s);result=mutator(s);});ensureState(state);emitShip(state);return {state,result};}
  ...
  function cargoLostFromHoldInState(state,roomId){
    const room=state.storage?.rooms?.[roomId];if(!room||room.type!=='storage')return {};
    const lost={};
    ...
    state.inventoryRevision=(Number(state.inventoryRevision)||0)+1;
    return lost;
  }
  ...
  function repairOne(targetId){
    return update(state=>{
      if(!carpenterFunctional(state))return {ok:false,reason:"Carpenter's Workshop is destroyed"};
  ...
  function repairAll(){
    return update(state=>{
      ...
      state.shipHealth.mast.hp=state.shipHealth.mast.max;for(const room of Object.values(state.shipHealth.rooms))room.hp=room.max;
      return {ok:true,cost:missing};
    }).result;
  }

  function roomAliveForStorage(state,roomId){const hp=state.shipHealth?.rooms?.[roomId]?.hp;return hp===undefined||hp>0;}
  function compactSlotNeed(state,totals){
    ...
    remainder.cannonballs=Math.max(0,(Number(totals.cannonballs)||0)-liveMagazine*Storage.ITEMS.cannonballs.stackSize);
    remainder.timber=Math.max(0,(Number(totals.timber)||0)-liveCarp*Storage.ITEMS.timber.stackSize);
    let general=0;for(const itemId of Object.keys(Storage.ITEMS))general+=Math.ceil(Math.max(0,Number(remainder[itemId])||0)/Storage.ITEMS[itemId].stackSize);
    return {general,holdSlots,fit:general<=holdSlots};
  }
  function physicalAdditional(itemId,amount,state){
    amount=Math.max(0,Math.floor(Number(amount)||0));if(!amount)return 0;let capacity=0;
    for(const [roomId,room] of Object.entries(state.storage?.rooms||{})){
      if(!roomAliveForStorage(state,roomId))continue;const rule=Storage.roomRule(room.type);if(!rule?.accepts?.includes(itemId))continue;
      for(const slot of room.slots||[]){if(!slot)capacity+=Storage.ITEMS[itemId].stackSize;else if(slot.item===itemId)capacity+=Math.max(0,Storage.ITEMS[itemId].stackSize-(Number(slot.quantity)||0));}
    }
  ...
  function repackBalancedInState(state){
    const totals=Storage.totals(state.storage),storage=state.storage;
    ...
    function placeDedicated(roomId,itemId){
      ...
      for(let i=0;i<room.slots.length&&left>0;i++){const q=Math.min(Storage.ITEMS[itemId].stackSize,left);room.slots[i]={item:itemId,quantity:q};left-=q;placed+=q;}
      return placed;
    }
    ...
    function placeAcrossHolds(itemId,quantity){
      let left=quantity;if(left<=0)return 0;const cap=Storage.ITEMS[itemId].stackSize;
      while(left>0){
        const available=holdIds.filter(id=>nextFree(id)>=0);if(!available.length)break;
        const round=Math.min(left,available.length*cap),base=Math.floor(round/available.length),extra=round%available.length;
    ...
    for(const itemId of ['medicine','food','cannonballs','timber']){const placed=placeAcrossHolds(itemId,remainders[itemId]);storage.unassigned[itemId]=remainders[itemId]-placed;}
    state.stores=Storage.totals(storage);return storage;
  }
  function placePhysicalInState(state,itemId,amount){
    let left=Math.max(0,Math.floor(Number(amount)||0));const roomIds=[];
    ...
    for(const roomId of roomIds){
      if(!roomAliveForStorage(state,roomId))continue;const room=state.storage.rooms[roomId],rule=Storage.roomRule(room.type);if(!rule?.accepts?.includes(itemId))continue;
  ...
  function addCapped(itemId,amount){
    return update(state=>{
      const wanted=Math.max(0,Math.floor(Number(amount)||0)),accepted=maxAdditional(itemId,wanted,state),left=wanted-accepted;
      if(accepted){if(state.shipSettings.autoBalanceCargo){state.storage.unassigned[itemId]=(Number(state.storage.unassigned[itemId])||0)+accepted;repackBalancedInState(state);}else placePhysicalInState(state,itemId,accepted);state.stores=Storage.totals(state.storage);bumpInventory(state);}
  ...
  function acceptReward(reward={}){
    ...
    update(state=>{
      ...
      for(const [itemId,raw] of Object.entries(reward.stores||{})){
        ...
        if(fit){if(state.shipSettings.autoBalanceCargo){state.storage.unassigned[itemId]=(Number(state.storage.unassigned[itemId])||0)+fit;repackBalancedInState(state);}else placePhysicalInState(state,itemId,fit);}
  ...
  function purchaseBundle(itemId,amount,cost){
    return update(state=>{
      ...
      state.coins-=price;
      if(state.shipSettings.autoBalanceCargo){state.storage.unassigned[itemId]=(Number(state.storage.unassigned[itemId])||0)+wanted;repackBalancedInState(state);}else placePhysicalInState(state,itemId,wanted);
      state.stores=Storage.totals(state.storage);bumpInventory(state);return {ok:true,accepted:wanted,cost:price};
    }).result;
  }
  function markCombatReportResolved(extra={}){return update(state=>{if(!state.lastCombat)return {ok:false};state.lastCombat={...state.lastCombat,...extra,rewardResolved:true};state.lastSeenCombatAt=state.lastCombat.endedAt||Date.now();return {ok:true};}).result;}
  function advanceDay(days){return update(state=>{state.day+=Math.max(0,Math.round(Number(days)||0));return state.day;}).result;}
  ...
  Adventure.save=function(state){
    if(state&&typeof state==='object'){
      ...
      if((Number(latest.inventoryRevision)||0)>(Number(state.inventoryRevision)||0)){
        state.inventoryRevision=latest.inventoryRevision;state.storage=clone(latest.storage);state.stores={...latest.stores};state.coins=latest.coins;state.trade=clone(latest.trade);state.shipSettings={...latest.shipSettings};state.shipHealth=clone(latest.shipHealth);state.scoreStats=clone(latest.scoreStats);
      }
    }
    return publicSave(state);
  };

  window.HighSeasShipVoyage={HEALTH_DEF,HOLD_IDS,ensureState,load,missingBlips,carpenterFunctional,roomHealth,repairOne,repairAll,applyToCombat,captureCombat,maxAdditional,addCapped,rebalance,setAutoBalance,acceptReward,purchaseBundle,markCombatReportResolved,advanceDay,recordCombatScore,scoreSummary};
})();